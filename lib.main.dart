...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> loadMarket() async {
    try {
      ...
      setState(() {
        loading = false;
        status = 'تعذر الاتصال — تحقق من الإنترنت';
      });
    }
  }
  ...
  void connectStream() {
    ...
    final url = 'wss://stream.binance.com:9443/ws/'
        '${restSymbol.toLowerCase()}@kline_$interval';
  ...
  double ema(List<double> values, int period) {
    ...
    final k = 2 / (period + 1);
    final count = math.min(period, values.length);

    double e = values.take(count).reduce((a, b) => a + b) / count;
  ...
  Signal buildSignal(List<Candle> cs) {
    if (cs.length < 25) {
      return Signal('WAIT', price, price, price, price, price, 0);
    }
    ...
    if (bullish) {
      return Signal(
        ...
        entry + dist * 1.8,
        entry + dist * 2.6,
        math.min(95, 68 + ((e9 - e21) / entry * 5000)),
      );
    }
    ...
    if (bearish) {
      return Signal(
        ...
        entry - dist * 1.8,
        entry - dist * 2.6,
        math.min(95, 68 + ((e21 - e9) / entry * 5000)),
      );
    }
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          const CircleAvatar(
                            backgroundColor: Color(0xFF1C2940),
                          ...
                          Text(
                            price == 0
                                ? '--'
                                : price.toStringAsFixed(price > 100 ? 2 : 6),
                            style: const TextStyle(
                              fontSize: 21,
                              fontWeight: FontWeight.bold,
                            ),
                      ...
                      SizedBox(
                        ...
                        child: ListView(
                          ...
                          children: ['1m', '5m', '15m', '1h', '4h', '1d'].map((x) {
                            return Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        LinearProgressIndicator(
                          value: (s.strength / 100).clamp(0, 1),
                          minHeight: 8,
                          borderRadius: BorderRadius.circular(10),
                        ),
                        const SizedBox(height: 8),
                        const Text(
                          'الإشارة تجريبية وليست ضمانًا للربح.',
  ...
  Widget infoRow(String title, double value, Color color) {
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(
            value.toStringAsFixed(value > 100 ? 2 : 6),
            style: TextStyle(
              color: color,
              fontWeight: FontWeight.bold,
            ),
...
class CandlePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final visible = candles.length > 55
        ? candles.sublist(candles.length - 55)
        : candles;

    final minP = visible.map((c) => c.low).reduce(math.min);
    final maxP = visible.map((c) => c.high).reduce(math.max);

    final range = (maxP - minP) == 0 ? 1 : maxP - minP;

    double y(double p) {
      return size.height - ((p - minP) / range) * (size.height - 20) - 10;
    }

    final grid = Paint()
      ..color = const Color(0xFF1B2939)
      ..strokeWidth = 1;

    for (int i = 1; i < 6; i++) {
      final yy = size.height * i / 6;
      canvas.drawLine(
        Offset(0, yy),
    ...
    if (signal != null && signal!.side != 'WAIT') {
      final levels = [
        (signal!.sl, Colors.redAccent, 'SL'),
        (signal!.entry, Colors.white, 'ENTRY'),
        (signal!.tp1, Colors.greenAccent, 'TP1'),
        (signal!.tp2, Colors.greenAccent, 'TP2'),
        (signal!.tp3, Colors.greenAccent, 'TP3'),
      ];
      ...
      for (final l in levels) {
        ...
        tp.paint(canvas, Offset(5, yy - 12));
      }
    }
  }
